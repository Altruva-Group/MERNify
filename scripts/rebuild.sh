...
docker compose up -d --build --no-deps shard01 shard01-secondary shard01-arbiter shard02 shard02-secondary shard02-arbiter shard03 shard03-secondary shard03-arbiter kafka zookeeper prometheus elasticsearch redis

echo "Waiting for 15 seconds..."
sleep 15

echo "Pulling second wave..."
docker compose up -d --build --no-deps kafka kibana configsvr01 configsvr02 configsvr03 router01 router02

echo "Waiting for 5 minutes..."
sleep 120

echo "Pulling third wave..."
docker compose up -d --build --no-deps logstash
...
docker compose up -d --build --no-deps api01 api02

echo "Waiting for 15 seconds..."
sleep 15

echo "Pulling fifth wave..."
docker compose up -d --build --no-deps client nginx
